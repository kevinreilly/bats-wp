...
class custom_post_widget extends WP_Widget {
	function custom_post_widget() {
		$widget_ops = array( 'description' => __( 'Displays Specific Sidebar Element', 'custom-post-widget' ) );
		$this->WP_Widget( 'custom_post_widget', __( 'Sidebar Element', 'custom-post-widget' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'custom_post_id' ); ?>"> <?php echo __( 'Sidebar Element to Display:', 'custom-post-widget' ) ?>
				...
				<?php
					...
					if ($content_block) {
						...
						echo '<option value="">' . __( 'No sidebar elements available', 'custom-post-widget' ) . '</option>';
					};
				?>
				</select>
			</label>
		...
		<p>
			<?php
				echo '<a href="post.php?post=' . $custom_post_id . '&action=edit">' . __( 'Edit Sidebar Element', 'custom-post-widget' ) . '</a>' ;
			?>
		</p>
...
function my_content_block_post_type_init() {
	$labels = array(
		'name' => _x( 'Sidebar Elements', 'post type general name', 'custom-post-widget' ),
		'singular_name' => _x( 'Sidebar Element', 'post type singular name', 'custom-post-widget' ),
		'plural_name' => _x( 'Sidebar Elements', 'post type plural name', 'custom-post-widget' ),
		'add_new' => _x( 'Add Sidebar Element', 'block', 'custom-post-widget' ),
		'add_new_item' => __( 'Add New Sidebar Element', 'custom-post-widget' ),
		'edit_item' => __( 'Edit Sidebar Element', 'custom-post-widget' ),
		'new_item' => __( 'New Sidebar Element', 'custom-post-widget' ),
		'view_item' => __( 'View Sidebar Element', 'custom-post-widget' ),
		'search_items' => __( 'Search Sidebar Elements', 'custom-post-widget' ),
		'not_found' =>  __( 'No Sidebar Elements Found', 'custom-post-widget' ),
		'not_found_in_trash' => __( 'No Sidebar Elements found in Trash', 'custom-post-widget' ),
		'parent_item_colon' => ''
	);
	$options = array(
		'labels' => $labels,
...
function content_block_messages( $messages ) {
	$messages['content_block'] = array(
		0 => '', 
		1 => current_user_can( 'edit_theme_options' ) ? sprintf( __( 'Sidebar Element updated. <a href="%s">Manage Widgets</a>', 'custom-post-widget' ), esc_url( 'widgets.php' ) ) : sprintf( __( 'Content Block updated.', 'custom-post-widget' ), esc_url( 'widgets.php' ) ),
		...
		5 => isset($_GET['revision']) ? sprintf( __( 'Sidebar Element restored to revision from %s', 'custom-post-widget' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		6 => current_user_can( 'edit_theme_options' ) ? sprintf( __( 'Sidebar Element published. <a href="%s">Manage Widgets</a>', 'custom-post-widget' ), esc_url( 'widgets.php' ) ) : sprintf( __( 'Sidebar Element published.', 'custom-post-widget' ), esc_url( 'widgets.php' ) ),
		7 => __( 'Block saved.', 'custom-post-widget' ),
		8 => current_user_can( 'edit_theme_options' ) ? sprintf( __( 'Sidebar Element submitted. <a href="%s">Manage Widgets</a>', 'custom-post-widget' ), esc_url( 'widgets.php' ) ) : sprintf( __( 'Sidebar Element submitted.', 'custom-post-widget' ), esc_url( 'widgets.php' ) ),
		9 => sprintf( __( 'Sidebar Element scheduled for: <strong>%1$s</strong>.', 'custom-post-widget' ), date_i18n( __( 'M j, Y @ G:i' , 'custom-post-widget' ), strtotime(isset($post->post_date) ? $post->post_date : null) ), esc_url( 'widgets.php' ) ),
		10 => current_user_can( 'edit_theme_options' ) ? sprintf( __( 'Sidebar Element draft updated. <a href="%s">Manage Widgets</a>', 'custom-post-widget' ), esc_url( 'widgets.php' ) ) : sprintf( __( 'Sidebar Element draft updated.', 'custom-post-widget' ), esc_url( 'widgets.php' ) ),
	);
	return $messages;
}
...
function add_content_block_icon() {
	...
	<a id="add-content-block" class="button thickbox" title="' . __("Add Sidebar Element", 'custom-post-widget' ) . '" href="' . plugins_url() . 'popup.php?type=add_content_block_popup&amp;TB_inline=true&amp;inlineId=content-block-form">
		<span class="wp-media-buttons-icon"></span>' . __("Add Sidebar Element", "custom-post-widget") . '</a>';
}
